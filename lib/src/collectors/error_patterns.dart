...
({String title, RenderIssueKind kind, IssueSeverity severity})?
    matchLayoutRender(String msg) {
  if (msg.contains('overflowed by') || msg.contains('RenderFlex overflowed')) {
    return (
      title: 'Render overflow',
...
String? matchLifecycle(String msg) {
  if (msg.contains('called after dispose()')) {
    return 'setState after dispose';
  }
  if (msg.contains('used after being disposed')) {
    return 'Object used after dispose';
  }
  if (msg.contains('has been unmounted') ||
      msg.contains('!_debugLifecycleState')) {
    return 'Access after unmount';
  }
  if (msg.contains('Another exception was thrown')) {
    return 'Cascading exception';
  }
  return null;
}
