...
void main() {
  test('overflow yields only a layoutRender issue when renders enabled',
      () async {
    ...
    expect(cats, isNot(contains(IssueCategory.exception)),
        reason:
            'no generic exception duplicate when render collector handles it');
  });

  test('overflow still yields an exception issue when renders disabled',
      () async {
    final store = RuntimeStore();
